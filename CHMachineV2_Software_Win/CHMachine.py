...
class motorclass():
    ...
    def detect(self):
        ...
        while 1:  
            ...
            while self.state==3 and arduino_connected==True: ##################### ALWAYSON/PATTERN/SRT
...
def init_setup(file):
    ...
    try:
        ...
        while 1:
            ...
            linelist=setup.readline().replace('\t', '').replace(' ', '').strip().split('=') #read line, remove spaces and tabs, splits the string at the "=" sign
            ...
            if linelist[0] == 'Debug_mode(Bool)':
                debug=bool(int(linelist[1]))
                if debug == True:
                    print('***DEBUG MODE***')
...
init_setup('setup.txt') #assign keys from setup.txt
arduino=None
