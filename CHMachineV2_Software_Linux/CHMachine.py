...
class MotorClass:
    ...
    def __init__(self):
        ...
        self.savestate = 2
        self.result = 0
        self.colorshow = np.zeros((streamwindowssizex, streamwindowssizey, 4),
                                  np.uint8)  # create an array of zeros for the black background
        self.index = 0
        self.listindex = 0
        self.timetic = 0
        self.patternspeed = 0
    ...
    def detect(self):
        ...
        while detectflag:
            ...
            while (self.state == 5) or (self.state == 1):  # ######### DETECT/DETECT SETUP
                ...
                if self.state == 5 and self.getspeed() != 0 and arduino_connected:
                    self.PWMpin('0')

                try:  # take care of sliding too fast which result in a shape out of boundaries
                    self.monitor = {"top": top, "left": left, "width": int(screenshotsizex),
                                    "height": int(screenshotsizey)}
                    ...
                    if checkinv == False:  # invert flag False
                        ...
                        if (self.result >= threshold):  # if match value is over the threshold
                            ...
                            self.colorshow = np.zeros((streamwindowssizex, streamwindowssizey, 4),
                                                      np.uint8)  # create a black background
                            cv2.circle(self.colorshow, (0, 0), 70, (0, 255, 0), -1)  # draw a green circle
                            self.tempo = pygame.time.get_ticks()

                        elif (pygame.time.get_ticks() - self.tempo) >= (
                                timeonvar):  # turn the pin to floor speed some time after the last match is occurred
                ...
                if stream_window_open == False:  # open "stream" window if isn't already opened
                    ...
                    stream_canvas = Canvas(stream_window, width=streamwindowssizex, height=streamwindowssizey,
                                           background='black')
                    stream_canvas.pack()
    ...
    def PWMpin(self, PWM_speed):  # set the Arduino pin PWM
        ...
        try:
            ...
            if ((pygame.time.get_ticks() - self.pinresttime) > self.serialfloodlimit or PWM_speed != self.speed):  # limit serial flooding
...
def init_setup(file):
    ...
    try:
        ...
        for x in range(100):
            linelist = setup.readline().replace('\t', '').replace(' ', '').strip().split('=')  # read line, remove spaces and split the string a the "=" sign
            ...
            if linelist[0] == 'Speedup_button_step(0-255)':
                speedup_step = int(linelist[1])
            ...
            if linelist[0] == 'Detect_Window_size':
                ...
                streamwindowssizey = int(linelist[1])

            if linelist[0] == 'Serial_Baud_Rate':
                serialbaud = int(linelist[1])
        ...
        for x in range(1000):
            ...
            try:
                ...
                if linelist.count('=') == 1 and linelist.count(':') > 0:
                    linelist = linelist.replace(' ', '').replace(',', '.').strip().split(
                        '=')  # read line, remove spaces, convert "," to "." and split the string at the "=" sign

                    if linelist[0] != '' and linelist[1] != '':
                        namelist.append(linelist[0][0:18])
                        stringlist = linelist[1].split(':')
                        intlist = [int(round(float(i))) for i in
                                   stringlist]  # converts list of strings into rounded integers
...
def autoserialstart(baud):
    ...
    for p in ports:
        ...
        try:
            ...
            print(p[0] + '...')
            arduino = serial.Serial(p[0], baud, timeout=1,
                                    write_timeout=1)  # always a good idea to specify a timeout in case we send bad data
            pygame.time.wait(5000)  # wait for Arduino to initialize

            arduino.write(('CHMT\n').encode('utf-8'))
            pygame.time.wait(1000)  # wait for a response from Arduino
...
def serialstart(COMstring, baud):
    ...
    if COMstring == ('') or COMstring == (
            'COM Port'):  # if no port is specified start autoserialstart() to find it automatically
    ...
    # manual port:
        ...
        try:
            arduino = serial.Serial(COMstring, baud, timeout=1,
                                    write_timeout=1)  # 2=Com3 on windows always a good idea to specify a timeout in case we send bad data
            pygame.time.wait(5000)  # wait for the Arduino to initialize
...
def onKeyDown(event):
    ...
    if event.Key == (slowdownbutton):
        ...
        if (checkAOVar.get() == True or checkPULVar.get() == True or checkDETVar.get() == True):
            if speedint > speeddown_step:
                speedint -= speeddown_step
    ...
    if event.Key == (speedupbutton):
        speedint = int(speed)
        if (checkAOVar.get() == True or checkPULVar.get() == True or checkDETVar.get() == True):
            if speedint <= 255-speedup_step:
                speedint += speedup_step
                motorspeed.set(speedint)
                speed = str(speedint)
    ...
    if (event.Key == screenshotbutton or event.Key == refreshbutton):
        ...
        if (event.Key == screenshotbutton):
            mousedata = Xlib.display.Display().screen().root.query_pointer()._data
        ...
        if (pos != [-1, -1]):
            ...
            base = np.zeros((streamwindowssizex, streamwindowssizey, 4),
                            np.uint8)  # an array of zeros for a black background
            x_offset = int((streamwindowssizex - screenshotsizex) / 2)
            y_offset = int((streamwindowssizey - screenshotsizey) / 2)
            base[y_offset:y_offset + arrbase.shape[0],
            x_offset:x_offset + arrbase.shape[1]] = arrbase  # center the image array
            ...
            if match_window_open == False:  # open "match" window if isn't already opened
                ...
                match_canvas = Canvas(match_window, width=streamwindowssizex, height=streamwindowssizey,
                                      background='black')
                match_canvas.pack()
    ...
    if event.Key == (savebutton):
        ...
        filesname = glob.glob(
            os.path.abspath(os.path.dirname(__file__)) + "/*.npz")  # find name of all .npz files in the main folder
        savelist = []
        ...
        if savelist != []:
            savename = (os.path.abspath(os.path.dirname(__file__)) + '/save' + str(
                max(savelist) + 1) + '.npz')  # find the max value to add to the string

        else:
            savename = (os.path.abspath(os.path.dirname(__file__)) + '/save0.npz')

        np.savez(savename, arrbase, pos, int(screenshotsizex), int(screenshotsizey), speed, floorspeed, timeonvar,
                 timeoffvar, threshold, checkinv)
    ...
    if event.Key == (loadbutton):
        ...
        if loadlist != []:
            ...
            loadname = (os.path.abspath(os.path.dirname(__file__)) + '/save' + str(
                loadlist.pop()) + '.npz')  # pop() removes last element and return it
            loaded_arrays = np.load(loadname)

            load_state(loaded_arrays['arr_0'], loaded_arrays['arr_1'], loaded_arrays['arr_2'], loaded_arrays['arr_3'],
                       loaded_arrays['arr_4'],
                       loaded_arrays['arr_5'], loaded_arrays['arr_6'], loaded_arrays['arr_7'], loaded_arrays['arr_8'],
                       loaded_arrays['arr_9'])
...
def on_closing():
    ...
    motor.stop()
    try:
        arduino.write('CHMSTOP').encode('utf-8')
        arduino.close()
    except:
        pass
    detectflag = False
...
slidera = Scale(root, from_=0, to=255, orient=HORIZONTAL, length=400.00, variable=motorspeed, label='MOTOR SPEED:',
                command=speedslider)
slidera.grid(columnspan=6, pady=5)
speed = (str(motorspeed.get()))

timeON = IntVar(value=200)
sliderb = Scale(root, from_=10, to=1000, orient=HORIZONTAL, length=400.00, variable=timeON, label='TIME ON(ms):',
                command=timeONslider)
sliderb.grid(columnspan=7, pady=5)
timeonvar = timeON.get()

timeOFF = IntVar(value=100)
sliderc = Scale(root, from_=10, to=1000, orient=HORIZONTAL, length=400.00, variable=timeOFF, label='TIME OFF(ms):',
                command=timeOFFslider)
sliderc.grid(columnspan=8, pady=5)
timeoffvar = timeOFF.get()

floorspeedVAR = IntVar(value=0)
sliderd = Scale(root, from_=0, to=255, orient=HORIZONTAL, length=400.00, variable=floorspeedVAR, label='FLOOR SPEED:',
                command=floorspeedslider)
sliderd.grid(columnspan=9, pady=5)
floorspeed = str(floorspeedVAR.get())

sizex = IntVar(value=25)
slidersizex = Scale(root, from_=1, to=100, orient=HORIZONTAL, length=400.00, variable=sizex, label='Xsize:',
                    command=slidersize)
slidersizex.grid(columnspan=10, pady=5)
screenshotsizex = sizex.get() * (streamwindowssizex - 20) / 100

sizey = IntVar(value=25)
slidersizey = Scale(root, from_=1, to=100, orient=HORIZONTAL, length=400.00, variable=sizey, label='Ysize:',
                    command=slidersize)
slidersizey.grid(columnspan=11, pady=5)
screenshotsizey = sizey.get() * (streamwindowssizey - 20) / 100

thresh = IntVar(value=70)
sliderthresh = Scale(root, from_=1, to=100, orient=HORIZONTAL, length=400.00, variable=thresh, label='THRESHOLD:',
                     command=thresholdslider)
...
arduino = None
init_setup(os.path.abspath(os.path.dirname(__file__)) + '/setup.txt')  # assign preferences from setup.txt
